...
const generateAndGetMealPlan = async (req, res) => {
  try {
    const userId = req.user.id;
    const user = await User.findById(userId);
    if (!user) {
    ...
    const fetchedMealPlan = await MealPlan.findOne({ user: userId });

    if (!fetchedMealPlan) {
      return res.status(404).json({ message: 'No meal plan found for this user' });
    }

    res.status(200).json({ message: `Subscription period left: ${remainingDays} days`, mealPlan: fetchedMealPlan });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
};
